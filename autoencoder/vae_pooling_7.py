...
if __name__ == '__main__':
    ...
    pca = PCA(n_components=3)  # Adjust the number based on how much dimensionality you want to reduce to
    pca_embeddings = pca.fit_transform(new_embeddings)

    # Compute the score for each node based on the magnitude in the PCA-reduced space
    scores = np.linalg.norm(pca_embeddings, axis=1)

    # Compute the mean score
    mean_score = np.mean(scores)

    # Nodes with scores higher than the mean might be considered more important.
    important_nodes = np.where(scores > mean_score)[0]
    print("Important nodes:", important_nodes)

    # Compute distances from all nodes to important nodes
    distances = cdist(new_embeddings, new_embeddings[important_nodes])

    # Define your radius threshold (adjust according to your requirements).
    RADIUS_THRESHOLD = 0.2

    # Initialize labels with a default value of -1
    labels = np.full(distances.shape[0], -1)

    for i, node_distances in enumerate(distances):
        # Get the indices where the distance is less than the threshold
        within_threshold = np.where(node_distances <= RADIUS_THRESHOLD)[0]

        # If there are any nodes within the threshold, get the closest one
        if len(within_threshold) > 0:
            closest_important_node = within_threshold[np.argmin(node_distances[within_threshold])]
            labels[i] = closest_important_node
    ...
    def visualize_clusters(data, labels, original_pdb_path):
        ...
        # For writing to CSV
        with open('beads_to_atoms_2.csv', 'w', newline='') as csvfile:
            beadwriter = csv.writer(csvfile)
            beadwriter.writerow(["BeadID", "ImportantAtom", "AllAtomsInCluster"])
            ...
            for label, nodes in clusters.items():
                ...
                try:
                    ...
                    for node in nodes:
                        centroid_atoms.append(
                            f"{atom_info[node + 1]['atom_name']} ({atom_info[node + 1]['residue_name']})")
                ...
                # Write to CSV
                beadwriter.writerow([f"B{label:03d}", most_common_atom_type, "; ".join(centroid_atoms)])
            ...
            io.save("coarse_grained_graph.pdb_12")
