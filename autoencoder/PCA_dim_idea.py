...
def estimate_latent_dim(data_list, variance_threshold=0.95):
    """
    Estimates the innermost dimension of VAE using PCA - NO IDEA IF THIS WILL WORK OR NOT!

    """
    # Convert the data to a suitable format - might need to think about this more
    node_features = []
    for data in data_list:
        node_features.append(data.x)
    node_features = torch.cat(node_features, dim=0).numpy()
