...
def load_data(graph_files):
    ...
    for graph_file in graph_files:
        try:
            ...
            coordinates = data.x[:, :3]
            new_x = torch.cat((data.x, coordinates), dim=1)
            new_data = Data(x=new_x, edge_index=build_graph(new_x))
...
def predict_and_reconstruct_graph(filepath, vae_models, output_file):
    ...
    for i, model in enumerate(vae_models):
        ...
        with torch.no_grad():
            ...
            # Add x, y, z coordinates as node features
            coordinates = coarse_grained_x[:, :3]
            new_x = torch.cat((coarse_grained_x, coordinates), dim=1)
            new_data = Data(x=new_x, edge_index=coarse_grained_edge_index)

            # Generate reconstructed graph
            reconstructed_x = model.decode(z, new_data.edge_index)
